...
static inline make_EHelper(op_fp) {
  switch (s->isa.instr.fpu.funct5) {
    /* b00 */ EX  (0, fadd)       EX(1, fsub)      EX  (2, fmul) EX(3, fdiv) EX  (4, fsgnj) EX(5, fmin_max)
    /* b01 */ EX  (8, fcvt_F_to_F)                               EX(11, fsqrt)
    /* b10 */                                                                IDEX(20, F_fpr_to_gpr, fcmp)
    /* b11 */ IDEX(24, F_fpr_to_gpr, fcvt_F_to_G)  IDEX(26, F_gpr_to_fpr, fcvt_G_to_F)  IDEX(28, F_fpr_to_gpr, fmv_F_to_G)  IDEX(30, F_gpr_to_fpr, fmv_G_to_F)
    default: exec_inv(s)
  }
}
// RVC
...
static inline void exec(DecodeExecState *s) {
  ...
  if (s->isa.instr.r.opcode1_0 == 0x3) {
    switch (s->isa.instr.r.opcode6_2) {
      IDEX (000, I, load)    IDEX (001, F_ld, fp_load)                            EX   (003, fence)
      IDEX (004, I, op_imm)  IDEX (005, U, auipc)       IDEX (006, I, op_imm32)
      IDEX (010, S, store)   IDEX (011, F_st, fp_store)                           IDEX (013, R, atomic)
      IDEX (014, R, op)      IDEX (015, U, lui)         IDEX (016, R, op32)
      IDEX (020, F_R, fmadd) IDEX (021, F_R, fmsub)     IDEX (022, F_R, fnmsub)   IDEX (023, F_R, fnmadd)
      IDEX (024, F_R, op_fp)
...
rvc: ;
    ...
    switch (rvc_opcode) {
      IDEX (000, C_ADDI4SPN, addi)IDEX(001, C_FLD, fp_ld)  IDEXW(002, C_LW, lds, 4)  IDEXW(003, C_LD, ld, 8)
      ...
      EX   (024, misc)      IDEX(025, C_FLDSP, fp_ld)        IDEXW(026, C_SWSP, st, 4) IDEXW(027, C_SDSP, st, 8)
      default: exec_inv(s);
    }
  }
}
