...
#define require(x) if (!(x)) raise_intr(s, EX_II, cpu.pc);
...
#define RM ({ int rm = s->isa.instr.fpu.rm; \
              if(rm == 7) rm = frm->val; \
              if(rm > 4) assert(0); \
              rm; })
...
#define BUILD_EXEC_F(x) \
  make_EHelper(concat(f, x)) { \
    ...
    { \
    case 8: \
      s0 = concat(f64_, x)(f64(id_src1->val), f64(id_src2->val)).v; \
      print_asm_fpu_template_3(concat(f, x), d); \
      break; \
    case 4: \
      s0 = concat(f32_, x)(f32(id_src1->val), f32(id_src2->val)).v; \
...
#define BUILD_EXEC_FM(func) \
  make_EHelper(func){ \
    require_fp; \
    softfloat_roundingMode = RM; \
    s1 = fpreg_l(s->isa.instr.fpu.funct5); \
    switch (s->width) \
    { \
    case 8: \
      s0 = func(64, id_src1->val, id_src2->val, s1); \
      print_asm_fpu_template_4(func, d); \
      break; \
    case 4: \
      s0 = func(32, id_src1->val, id_src2->val, s1); \
...
#define print_asm_fpu_template_4(inst, fmt) \
  print_asm(str(inst) "." str(fmt) "%c %s,%s,%s,%s", \
    suffix_char(id_dest->width), id_src1->str, id_src2->str, fpreg_name(s->isa.instr.fpu.funct5, 4), id_dest->str)
